...
def init_database():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Add fee_type to existing payment_records table
            cur.execute("""
...
def json_exists(filename):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT 1 FROM json_store WHERE filename=%s LIMIT 1",
                (filename,)
            )
...
def json_load(filename):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT data FROM json_store WHERE filename=%s",
                (filename,)
            )
            row = cur.fetchone()
...
def json_delete(filename):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM json_store WHERE filename=%s",
                (filename,)
            )
...
def install_json_compatibility():
    ...
    def db_open(
        ...
        **kwargs
        ...
        if (
            ...
            and str(file).lower().endswith(".json")
        ):
            ...
            if (
                "r" in mode
                and not json_exists(filename)
            ):
                raise FileNotFoundError(filename)

            if (
                "x" in mode
                and json_exists(filename)
            ):
                raise FileExistsError(filename)

            return _DatabaseJSONFile(
                filename,
                mode
            )
    ...
    def db_exists(path):
        ...
        if (
            ...
            and str(path).lower().endswith(".json")
        ):
...
# Initialize once when the Flask process starts,
# then redirect JSON access to PostgreSQL.
